...
```*.sh-session
   npm run newacct
      create account specified in .env
   npm run follow
      follow accounts from did.txt
   npm run followback
      log in to accounts in handle.txt, follow NEWHANDLE in .env
   npm run enableadult
      log into new account from .env, enable adult content
   npm run auto
      run all scripts
```
<!-- 
